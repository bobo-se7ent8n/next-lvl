 ...
 */
export function useCountUp(value: number, key: unknown = null): number {
 ...
 */
export function useCountUpText(value: string, key: unknown = null): string {
  const numeric = Number.parseFloat(value);
  const animatable = Number.isFinite(numeric);
  const decimals = animatable && value.includes('.') ? (value.split('.')[1] ?? '').length : 0;
  const counted = useCountUp(animatable ? numeric : 0, key);
